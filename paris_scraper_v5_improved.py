...
class ParisScraperV5Improved(BaseScraperV5):
    ...
    async def _scrape_with_port_logic(self, page: Page, url: str, max_products: int) -> List[ProductData]:
        ...
        while len(all_products) < max_products:
            try:
                ...
                # Scroll para cargar productos (lógica PORT)
                await self._progressive_scroll_port_style(page)
                
                # Extraer productos de esta página
                page_products = await self._extract_products_port_method(page)
    ...
    def _build_page_url(self, base_url: str, page_num: int) -> str:
        """🔗 Construir URL con paginación para Paris"""
        if page_num <= 1:
            return base_url
